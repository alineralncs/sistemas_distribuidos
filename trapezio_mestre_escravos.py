...
def mestre(x0, xn, n, comm, rank, size):
    if n == 0:
        ...
        # Nessa função, cada processo (mestre) calcula a soma 
        # local da função nos seus subintervalos específicos, 
        # usando valores de x diferentes
        for i in range(1, n):
            local_sum += f(x)
            x += h
        # Redução 
...
def escravo(x0, xn, n, comm, rank, size):
    if n == 0:
        ...
        local_sum = 0.0
        x = x0 + (rank + 1) * h
        # Nessa função, cada processo escravo calcula a 
        # soma local da função nos seus subintervalos específicos, 
        # usando valores de x diferentes.
